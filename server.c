...
void * ClientFun(void *fd) {
    ...
    for (int i = 0; i < 2; ++i) {
        inf = read(p_clientKit->sock, p_tmpAr[i], sizeof(char) * STR_LEN);
    ...
    switch(inf) {
    case 0:
        ServerPrint(3, "\ruser ", username, " has logged in\n");
        inf = write(p_clientKit->sock, "success 1", sizeof("success 1"));
        break;
    case -1:
        ServerPrint(3, "\ruser ", username, " has tried to log in - wrong password\n");
        inf = write(p_clientKit->sock, "wrong password", sizeof("wrong password"));
        out = 0;
        break;
    case -2:
        if (AddUserToList(username, password, p_clientKit->registered)) {
            ...
            inf = write(p_clientKit->sock, "not available", sizeof("not available"));
            out = 0;
        }
        else {
            ServerPrint(3, "\ruser ", username, " has registered and logged in\n");
            inf = write(p_clientKit->sock, "success 2", sizeof("success 2"));
        }
        break;
    default:
        RemClient(p_clientKit);
    ...
    for(;;) {
        inf = read(p_clientKit->sock, receiveAr, sizeof(receiveAr));
        if (inf == -1 || !strcmp(receiveAr, "exit")) {
            RemClient(p_clientKit);
            if (inf == -1) {
        ...
        inf = write(p_clientKit->sock, transmitAr, sizeof(transmitAr));
        if (inf == -1) {
...
int CreateServer() {
    ...
    ClientKit termFunCSSK = { 0, 0, { 0 }, &registered };
