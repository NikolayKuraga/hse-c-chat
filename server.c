...
void* ClientFun(void* fd) {
    ...
    inf = recv(p_clientKit->clientSock, username, sizeof(username), 0);
    if (inf == -1) {
    ...
    inf = recv(p_clientKit->clientSock, password, sizeof(username), 0);
    if (inf == -1) {
    ...
    case 2:
        if (AddUserToList(username, password, p_clientKit->registered)) {
            ...
            inf = send(p_clientKit->clientSock, "not available", sizeof("not available"), 0);   // FIX THIS SHIT!
            free(p_clientKit);
            return (void*)4;
        }
        ServerPrint(3, "user ", username, " shas registered and logged in\n");
...
int CreateServer() {
    ...
    ClientKit termFunCSSK = { 0, &active, &registered };
